...
Написан одинаковый код (хеширование строк разной длины с помощью алгоритма SHA256, шифрование с помошью RSA c разной длиной ключа) c использованием Bouncy Castle на Java и с помощью нашего Python-интерфейса.
...
При коротких строках Python имеет фиксированные накладные расходы, так как, помимо вызовов Java методов, он интерпретирует код со своей стороны и осуществляет пересылка TCP пакетов. При длинных строках вклад от интерпретации становится ниже, на первый план выходят накладные расходы от пересылки TCP пакетов, поэтому наклон у Python-кривой больше.
...
#### RSA
Сложность RSA растет не линейно с увеличение длины ключа, но мы получаем результаты аналогичные SHA256: при коротких ключах фисксированные накладные расходы, при увеличении ключей наклон у Python-кривой больше по тем же причинам. Видим также, что при больших ключах отношение производительности Python к производительности Java уменьшается (например, расшировка при 4096 имеет почти одинаковую производительность).
...
Итого: имеем наклданые расходы, связанные с пересылкой TCP-пакетов, но при трудоемких вычислениях накладные расходы становятся несущественными.

### Информация о машине, на которой запускались бенчмарки
```
System:
